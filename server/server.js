...
class Server {
	start(binPath, workingDir){
		...
		this.cp.on( 'error', err => {
			this.isRunning = false;
			console.error("failed to start server: ", err.toString());
			reject(err);
		} );
		...
		this.cp.stderr.on( 'data',  data => {
			if (!this.lastErrors) {
				this.lastErrors = [];
			} else if (this.lastErrors.length >= 10) {
		...
		this.cp.on( 'exit', () => {
			...
			try { fs.writeFileSync(crashReport, this.lastErrors.join("\n"), 'utf-8'); }
			catch(e) { console.log("failed to save a file"); }
