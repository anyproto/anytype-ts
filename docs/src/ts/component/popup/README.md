# popup/ - Modal Dialogs

Modal popup system with **36 TSX files** (plus 10 Storybook story files). Managed by the `PopupStore` in `src/ts/store/popup.ts`.

## Architecture

`index.tsx` is the popup container that:
- Maps popup IDs to React components via a registry
- Handles positioning (centered with sidebar awareness)
- Manages dimmer overlay (configurable per popup)
- Provides lifecycle: `close()`, `position()`, `storageGet/Set()`, `getId()`
- Supports animation with configurable timeouts

Open a popup: `S.Popup.open('confirm', { data: { ... } })`

## Popup Types

### Core
- `search.tsx` - Space & global search with virtualized results (~3200 lines): removable filter tokens inside the input (`tokensRef`, exclusivity groups scope/what/who/relation; Backspace at 0 pops the rightmost of the rendered order, row-added tokens carry Back-restore snapshots), the current Channel as a removable scope token (leftmost; removing it flips to vault-wide search in place, Cmd+Shift+K toggles it; `isGlobal` is derived from the tokens, `param.data.isGlobal` only seeds the initial scope; object pickers pin the scope; `/in` re-points the scope at ANOTHER Channel — a foreign scope keeps the token + that Channel's chips/people gates but rides the cross-space loaders with `spaceId Equal`, no highlights/creates/settings, space captions shown; Cmd+Shift+K/× from it go global; a saved scope survives a quick reopen on any entry in its saved pill position; without one the entry point seeds the slot, and the 5-minute stale reset returns to entry defaults), one unified storage key set (`tokens`/`filter`/`recentSort`/`lastUsed` with one-shot migrations from the split `*Global` and pre-token keys), adaptive suggestion row (only addable tokens: kind/type chips while the what group is empty, inline person chips ("By me" after Messages, "By <name>" members at the end) while no creator token; no selected state, row hides when empty), `/by` `/type` `/in` typed completions (empty `/by` = person browse, `/in` = Channel browse in vault order; a whitespace-preceded "/" mid-query enters the same command mode — `parseCommandQuery` — and resolving the command restores the query it was typed after), a cross-Channel Types aggregate (global Types bucket and global `/type` grouped by uniqueKey over `GLOBAL_DEPS.types`, "in <Channel> + N other Channels" captions, served in-memory), cross-chat message search (`ChatSearch`), sectioned groups leading the vault-wide text search — Channels (3), People and Types (3 for queries of 3 letters or fewer / 10 from four, `groupMatchLimit`), then an Objects header over the backend results (people matched over the cross-space participants subscription deduplicated by identity 1:1-first; types via the uniqueKey aggregate, excluded from the RPC on the plain-text path; a person and their 1:1 Channel are deliberately two rows — the Channel row keeps the open/search-inside verbs, the person row the person verbs: "member in N Channels" caption, creator drill, and a click that focuses the group (spec 2026-08-29: the what-token carries a `focus` payload — persisted as `{uniqueKey|identity, name}`, restored only into scope-less opens, representative re-resolved when the cross-space maps land; focused Types/member tokens list that uniqueKey's / identity's per-space instances synchronously from the cross-space maps, pill named after the focus, Back restores the pre-focus view; grouped rows inside the Types bucket focus the same way; the participant menu sits one click deeper on a concrete member row, its Connect button creating the 1:1 — never auto-created); participant objects are excluded from the cross-space RPC in global mode except for member-type queries), per-token create actions, a search-icon drill affordance on result rows (one hover-highlighted click zone; until the first drill — `searchDrill` onboarding key — the tooltip text and Shift+Enter/→ shortcuts print before the icon, action over shortcut), Tab walks a transient highlight over the suggestion chips (Enter applies), Cmd+L copies the web link / Cmd+Alt+L the app deeplink (the footer hint swaps to the deeplink alternate while Alt is held — the macOS menu pattern), a date-sectioned recents browse (Today / Yesterday / Previous 7 and 14 days / month+year, grouped by the active order's own field — stamped on the loaded mode so quiet-reload windows never bucket text-ordered rows; the first header states the full logic, "Pages edited Today", and carries the sort switch), and a hold-the-top guard pushing back late native scroll deliveries for 400ms after every list swap, an in-chat search Expand handoff (presetTokens/presetFilter → a chat-focused Messages token bound to its Channel scope, dying on scope changes), an empty-state Messages tutorial and "/is Message" completions
- `confirm.tsx` - Confirmation dialog with optional checkbox/input
- `preview.tsx` - Image/video gallery with Swiper carousel, zoom, thumbnails
- `export.tsx` - Export options (Markdown, Protobuf, PDF, HTML)
- `help.tsx` - What's New / documentation viewer
- `shortcut.tsx` - Keyboard shortcuts reference
- `upload.tsx` - File upload dialog

### Pages & Objects
- `page.tsx` - Full-page object viewer/editor in modal
- `objectManager.tsx` - Object management interface
- `cleanup.tsx` - Cascade-deletion confirmation: checkbox tree of orphan candidates (objects and files, nested by `createdInContext`) offered for archival. Opened from the `CleanupSuggestion` toast's "Review" action (`I.ToastAction.Cleanup`); confirms with `skipCascade=true`
- `relation.tsx` - Batch relation editor for multiple objects

### Auth & Security
- `pin.tsx` - PIN entry verification
- `phrase.tsx` - Recovery phrase information
- `logout.tsx` - Logout confirmation

### Spaces & Collaboration
- `space/create.tsx` - Space creation with icon selection
- `invite/request.tsx` - Space join request display
- `invite/confirm.tsx` - Invitation confirmation
- `invite/qr.tsx` - QR code invitation
- `invite/manage.tsx` - Manage the space invite: auto-approval, permissions, share-within-space, reset (owner only)
- `invite/add.tsx` - Add members directly by picking people you already share a space with

### Membership
- `membership/activation.tsx` - Activation flow
- `membership/finalization.tsx` - Finalization flow

### Onboarding
- `onboarding.tsx` - Tutorial with Swiper carousel
- `settings/onboarding.tsx` - Settings onboarding
- `usecase.tsx` - Use case selection (with `page/usecase/list.tsx`, `page/usecase/item.tsx`)
- `aiOnboarding.tsx` - AI features introduction (with `page/aiOnboarding/statusMessage.tsx`)
- `introduceChats.tsx` - Chat features intro

### Graph
- `graph/OnboardingGraphWorker.tsx` - Graph onboarding worker
- `dimmerWithGraph.tsx` - Dimmer overlay with graph visualization

### Other
- `about.tsx` - About/info popup
- `share.tsx` - App sharing link
- `api/create.tsx` - API token management
