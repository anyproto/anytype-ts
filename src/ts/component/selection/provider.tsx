...
const SelectionProvider = observer(
	class SelectionProvider extends React.Component<Props> {
		...
		componentDidMount() {
			...
			UtilCommon.getScrollContainer(isPopup).on(
				'scroll.selection',
				(e: any) => {
					this.onScroll(e);
				}
		...
		onMouseDown(e: any) {
			...
			isPopup
				? selectionRect.addClass('fromPopup')
				: selectionRect.removeClass('fromPopup');
			...
			win.on(`mousemove.selection`, (e: any) => {
				this.onMouseMove(e);
			});
			win.on(`blur.selection mouseup.selection`, (e: any) => {
				this.onMouseUp(e);
			});
		}
			...
			if (!this.moved) {
				if (!e.shiftKey && !e.altKey && !(e.ctrlKey || e.metaKey)) {
					...
					if (
						...
						const tree = blockStore.getTree(
							rootId,
							blockStore.getBlocks(rootId)
						);
		...
		checkNodes(e: any) {
			...
			const rect = UtilCommon.objectCopy(
			...
			for (const i in I.SelectType) {
				...
				for (const id of ids) {
					...
					if (type == I.SelectType.Block) {
						...
						const childrenIds = blockStore.getChildrenIds(
							rootId,
							id
						);
