	...
	class MenuRelationSuggest extends React.Component<I.Menu, State> {
			...
			const filters: any[] = [
				{
					...
					value: [commonStore.space, Constant.storeSpaceId],
				},
				{
					operator: I.FilterOperator.And,
					relationKey: 'layout',
		...
		onMouseEnter(e: any, item: any) {
			...
			if (
				...
				!menuStore.isAnimating(this.props.id)
			) {
		...
		onClick(e: any, item: any) {
			...
			if (item.id == 'add') {
				...
				if (item.isInstalled || noInstall) {
					...
					Action.install(item, true, (message: any) => {
						cb(message.details);
					});
				}
			}
		...
		getLibrarySources() {
			return this.items
				.filter(it => it.spaceId == commonStore.space)
