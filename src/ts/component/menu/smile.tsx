...
class MenuSmile extends React.Component<I.Menu, State> {
				...
				const Item = (item: any) => {
					...
					return item.itemId == ID_BLANK ? (
						...
						<div
							...
							onContextMenu={e =>
								this.onSkin(e, item.id, item.itemId)
							}
						>
							<div
		...
		if (!noHead) {
			if (!noGallery) {
				buttons = buttons.concat([
					{
						text: translate('menuSmileRandom'),
						onClick: this.onRandom,
					},
					{
						text: translate('menuSmileGallery'),
						onClick: () => this.onTab(Tab.Gallery),
						isActive: tab == Tab.Gallery,
					},
				]);
			}
			if (!noUpload) {
				buttons.push({
					text: translate('menuSmileUpload'),
					onClick: () => this.onTab(Tab.Upload),
					isActive: tab == Tab.Upload,
				});
			}
			if (!noRemove) {
				buttons.push({
	...
	getGroups() {
		return this.checkRecent(
			UtilSmile.getCategories().map(it => ({ id: it.id, name: it.name }))
		);
	}
	...
	getItems() {
		...
		const fillRowWithBlankChildren = row => {
			...
			if (len > 0 && len < LIMIT_ROW) {
				row.children.push(
					...new Array(LIMIT_ROW - len).fill({ itemId: ID_BLANK })
				);
			}
	...
	onKeyUp(e: any, force: boolean) {
		...
		this.timeoutFilter = window.setTimeout(
			() => {
				this.setState({
					page: 0,
					filter: UtilCommon.regexEscape(this.refFilter.getValue()),
				});
			},
			force ? 0 : 50
		);
	...
	setActive(item?: any, row?: number) {
		...
		if (this.active) {
			const element = node.find(
				`#item-${$.escapeSelector(this.active.id)}`
			);

			element.addClass('active');
			Preview.tooltipShow({
				text:
					UtilSmile.aliases[this.active.itemId] || this.active.itemId,
				element,
			});
		}
	}
	...
	onArrowVertical(dir: number) {
		...
		const firstBlank = current.children.findIndex(
			it => it.itemId == ID_BLANK
		);
	...
	onArrowHorizontal(dir: number) {
		...
		// Arrow right
		if (
			this.coll > current.children.length - 1 ||
			current.children[this.coll].itemId == ID_BLANK
		) {
			this.coll = 0;
			this.onArrowVertical(dir);
			return;
