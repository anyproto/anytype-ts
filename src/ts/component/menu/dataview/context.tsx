...
class MenuContext extends React.Component<I.Menu> {
	...
	onClick(e: any, item: any) {
		...
		const { data } = param;
		const { subId, objectIds, onSelect, targetId, isCollection, route } =
			data;
		const win = $(window);
		const count = objectIds.length;
		const first =
			count == 1 ? detailStore.get(subId, objectIds[0], []) : null;
		const cb = () => {
			if (onSelect) {
				onSelect(item.id);
			}
		...
		switch (item.id) {
			...
			case 'createWidget': {
				const firstBlock = blockStore.getFirstBlock(
					blockStore.widgets,
					1,
					it => it.isWidget()
				);
