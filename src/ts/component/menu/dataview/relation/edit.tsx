	...
	class MenuRelationEdit extends React.Component<I.Menu> {
						...
						<div className="section" onMouseEnter={this.menuClose}>
							<div className="inputWrap">
								<Button
									...
									type="input"
									text={translate(
										relation ? 'commonSave' : 'commonCreate'
									)}
									color="blank"
									className="c28"
								/>
		...
		getSections() {
			...
			if (relation) {
				canDuplicate =
					relation &&
					blockStore.checkFlags(rootId, blockId, [
		...
		onClick(e: any, item: any) {
			...
			const idx = view.relations.findIndex(
				it => it.relationKey == relation.relationKey
			);
		...
		isAllowed() {
			...
			let ret = relation
				? blockStore.isAllowed(relation.restrictions, [
						I.RestrictionObject.Details,
					])
				: true;
			if (ret) {
				ret = blockStore.checkFlags(rootId, blockId, [
