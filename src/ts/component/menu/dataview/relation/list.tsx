...
const MenuRelationList = observer(
	class MenuRelationList extends React.Component<I.Menu> {
		...
		render() {
			...
			const allowedView = blockStore.checkFlags(rootId, blockId, [
					...
					>
						...
						>
							<Icon
								className={
									'relation ' +
									Relation.className(item.relation.format)
								}
							/>
					...
					{!readonly && allowedView ? (
						<div className="bottom">
							...
							>
								...
								<div className="name">
									{translate(
										'menuDataviewRelationListAddRelation'
									)}
								</div>
							</div>
						</div>
		...
		onSortEnd(result: any) {
			...
			const relations = view.relations.filter(it => {
				...
				return (
					!relation.isHidden ||
					(relation.isHidden && it.relationKey == 'name')
				);
			});
		...
		resize() {
			...
			const height = Math.max(
				HEIGHT * 2,
				Math.min(360, items.length * HEIGHT + 58)
			);
