...
const MenuDataviewObjectList = observer(
	class MenuDataviewObjectList extends React.Component<I.Menu, State> {
		...
		getItems() {
			...
			const ret = UtilCommon.objectCopy(this.items).filter(
		...
		load(clear: boolean, callBack?: (message: any) => void) {
			...
			if (types && types.length) {
				const map = types
					.map(id => dbStore.getTypeById(id)?.uniqueKey)
					.filter(it => it);
				if (map.length) {
					filters.push({
						operator: I.FilterOperator.And,
			...
			UtilData.search(
				...
				(message: any) => {
					...
					this.items = this.items.concat(
						(message.records || []).map((it: any) => {
							it.name = String(
								it.name || UtilObject.defaultName('Page')
							);
							return it;
						})
					);
		...
		onClick(e: any, item: any) {
			...
			const cb = (id: string) => {
				...
				let value = UtilCommon.arrayUnique(
