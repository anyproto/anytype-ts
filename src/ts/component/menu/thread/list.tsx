...
const MenuThreadList = observer(
	class MenuThreadList extends React.Component<I.Menu> {
		node: any = null;
		timeoutMenu = 0;
		timeoutClose = 0;
		...
		render() {
			...
			const Item = (item: any) => (
				...
				>
					<IconObject
						object={{ ...item, layout: I.ObjectLayout.Human }}
					/>
					<div className="info">
						<div className="name">{item.name}</div>
						<div className="description">
							<div className="side left">
								{translate('menuThreadListLastSync')}
							</div>
							<div className="side right">
								{UtilDate.timeAgo(
									Math.max(item.lastPulled, item.lastEdited)
								)}
							</div>
						</div>
					</div>
