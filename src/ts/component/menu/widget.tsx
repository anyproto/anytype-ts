...
const MenuWidget = observer(
	class MenuWidget extends React.Component<I.Menu> {
		...
		checkState() {
			...
			if (this.isCollection()) {
				if ([null, I.WidgetLayout.Link].includes(this.layout)) {
					this.layout =
						this.target.id == Constant.widgetId.favorite
							? I.WidgetLayout.Tree
				...
				if (
					...
					!setLayouts.includes(this.target.layout)
				) {
			...
			const limitOptions = UtilMenu.getWidgetLimits(this.layout).map(it =>
				Number(it.id)
			);
		...
		isCollection() {
			return (
				this.target &&
				Object.values(Constant.widgetId).includes(this.target.id)
			);
		}
		...
		getTargetId(): string {
			...
			if (isEditing) {
				...
				const widgetIds = blockStore.getChildrenIds(
					blockStore.widgets,
					blockStore.widgets
				);
