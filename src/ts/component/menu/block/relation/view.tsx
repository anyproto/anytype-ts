...
const MenuBlockRelationView = observer(
	class MenuBlockRelationView extends React.Component<I.Menu> {
		...
		render() {
			...
			let allowedBlock = blockStore.checkFlags(rootId, rootId, [
				I.RestrictionObject.Block,
			]);
			let allowedRelation = blockStore.checkFlags(rootId, rootId, [
				I.RestrictionObject.Relation,
			]);
			let allowedValue = blockStore.checkFlags(rootId, rootId, [
		...
		componentDidMount() {
			...
			this.resize();
			scrollWrap.off('scroll').on('scroll', (e: any) => {
				this.onScroll();
			});
		...
		onFav(e: any, relationKey: string) {
			...
			const featured = UtilCommon.objectCopy(
		...
		resize() {
			...
			obj.css({
				height:
					container.height() - UtilCommon.sizeHeader() - maxOffset,
