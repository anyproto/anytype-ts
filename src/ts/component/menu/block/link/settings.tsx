...
const MenuBlockLinkSettings = observer(forwardRef<I.MenuRef, I.Menu>((props, ref) => {
	...
	const getSections = () => {
		...
		const itemName = { id: 'name', name: translate('menuBlockLinkSettingsName'), icon: Relation.registryIconName(I.RelationType.ShortText) };
		const itemDescription = canDescription ? {
			id: 'description', name: translate('menuBlockLinkSettingsDescription'), icon: Relation.registryIconName(I.RelationType.LongText),
			caption: description.name, arrow: true
		} : null;
		const itemType = { id: 'type', name: translate('commonObjectType'), icon: Relation.registryIconName(I.RelationType.Object), withSwitch: true, switchValue: hasRelationKey('type') };
