...
class MenuBlockAction extends React.Component<I.Menu, State> {
			...
			<div className="section">
				...
				<div className="items">
					{item.children.map((action: any, i: number) => {
						...
						if (action.isBgColor) {
							icn.push(
								'bgColor bgColor-' + (action.value || 'default')
							);
						}
	...
	componentDidMount() {
		...
		menu.off('mouseleave').on('mouseleave', () => {
	...
	getSections() {
		...
		if (filter) {
			const turnText = {
				...
				children: UtilMenu.getBlockText(),
			};
			const turnList = {
				id: 'turnList',
				icon: '',
				name: translate('menuBlockActionsSectionsListStyle'),
				children: UtilMenu.getBlockList(),
			};
			const turnPage = {
				id: 'turnPage',
				icon: '',
				name: translate('commonTurnIntoObject'),
				children: UtilMenu.getTurnPage(),
			};
			const turnDiv = {
				id: 'turnDiv',
				icon: '',
				name: translate('menuBlockActionsSectionsDividerStyle'),
				children: UtilMenu.getTurnDiv(),
			};
			const turnFile = {
				id: 'turnFile',
				icon: '',
				name: translate('menuBlockActionsSectionsFileStyle'),
				children: UtilMenu.getTurnFile(),
			};
			const action = {
				id: 'action',
				icon: '',
				name: translate('menuBlockActionsSectionsActions'),
				children: [],
			};
			const align = {
				id: 'align',
			...
			const color = {
				...
				arrow: true,
				children: UtilMenu.getTextColors(),
			};
			const dataview = {
				id: 'dataview',
				icon: '',
				name: translate('menuBlockActionsSectionsDataview'),
				children: UtilMenu.getDataviewActions(rootId, blockId),
			};
			...
			for (const id of blockIds) {
				...
				hasBookmark = block.isBookmark()
					? this.checkFlagByObject(targetObjectId)
					: false;
				hasDataview = block.isDataview()
					? this.checkFlagByObject(targetObjectId)
					: false;
			...
			if (hasDataview) {
				section2.children = section2.children.concat(
					UtilMenu.getDataviewActions(rootId, blockId)
				);
			}
			...
			if (hasTurnText) {
				const caption = I.TextStyle[style]
					? translate(
							UtilCommon.toCamelCase(
				...
				section2.children.push({
					id: 'turnStyle',
					icon: UtilData.styleIcon(I.BlockType.Text, style),
					name: translate('menuBlockActionsSectionsTextStyle'),
					arrow: true,
					caption,
				});
			...
			if (hasTurnDiv) {
				section2.children.push({
					id: 'turnStyle',
					icon: UtilData.styleIcon(I.BlockType.Div, style),
					name: translate('menuBlockActionsSectionsDividerStyle'),
					arrow: true,
					isDiv: true,
				});
			...
			if (hasAlign) {
				section2.children.push({
					id: 'align',
					icon: ['align', UtilData.alignIcon(align)].join(' '),
					name: translate('commonAlign'),
					arrow: true,
				});
			}
		...
		switch (item.itemId) {
			...
			case 'dataviewSource': {
				...
				if (isCollection) {
					...
					addParam.onClick = () => {
						C.ObjectCreateSet([], {}, '', commonStore.space, () =>
							onCreate()
						);
					};
		...
		switch (item.itemId) {
			...
			case 'openBookmarkAsObject': {
				UtilObject.openPopup({
					id: block.content.targetObjectId,
			...
			case 'openFileAsObject': {
				UtilObject.openPopup({
					id: block.content.hash,
					layout: I.ObjectLayout.File,
				});

				analytics.event('OpenAsObject', {
					type: block.type,
					params: { fileType: block.content.type },
				});
				break;
			}
			...
			case 'openDataviewFullscreen': {
				UtilObject.openPopup({
					layout: I.ObjectLayout.Block,
					id: rootId,
					_routeParam_: { blockId },
				});
			...
			case 'openDataviewObject': {
				UtilObject.openPopup(
					detailStore.get(rootId, block.content.targetObjectId)
				);
				break;
			}
