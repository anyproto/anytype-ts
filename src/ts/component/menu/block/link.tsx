	...
	class MenuBlockLink extends React.Component<I.Menu, State> {
		...
		onFilterChange(e: any) {
			...
			this.timeout = window.setTimeout(() => {
				menuStore.updateData(this.props.id, {
		...
		getSections() {
			...
			const isLocal =
				filter.match(/^file:/) || UtilCommon.matchLocalPath(filter);
			const isUrl =
				UtilCommon.matchUrl(filter) || UtilCommon.matchDomain(filter);
			const items = []
				.concat(this.items)
				.map(it => ({ ...it, isBig: true }));
			...
			const buttons: any[] = [
				isUrl || isLocal
					? {
							id: 'link',
							name: translate(
								'menuBlockLinkSectionsLinkToWebsite'
							),
							icon: 'link',
							isLocal,
						}
			...
			const filters: any[] = [
				{
					...
					value: UtilObject.getSystemLayouts(),
				},
			];
			const sorts = [
