...
const MenuBlockAdd = observer(
	class MenuBlockAdd extends React.Component<I.Menu> {
		...
		getRelations() {
			...
			const type = dbStore.getTypeById(
			...
			const ret = relations
				...
				.filter(it =>
					!config.debug.ho && it.isHidden ? false : it.isInstalled
				)
		...
		getSections() {
			...
			let sections: any[] = [
				...
				},
				{
					...
					children: UtilMenu.getBlockMedia(),
				},
				{
					id: 'embed',
					name: translate('menuBlockAddSectionsEmbed'),
					children: UtilMenu.getBlockEmbed(),
				},
				{
					id: 'other',
					name: translate('menuBlockAddSectionsOther'),
					children: UtilMenu.getBlockOther(),
				},
				{
					id: 'object',
					name: translate('menuBlockAddSectionsObjects'),
					children: UtilMenu.getBlockObject(),
				},
			].map(s => ({
				...s,
			...
			if (filter && filter.text) {
				...
				if (block.canHaveAlign()) {
					sections.push({
						...
						color: '',
						children: UtilMenu.getAlign(block.isTextQuote()),
					});
				}
		...
		onOver(e: any, item: any) {
			...
			const { filter } = commonStore;
			const block = blockStore.getLeaf(rootId, blockId);
			const text = UtilCommon.stringCut(
				data.text,
				filter.from - 1,
