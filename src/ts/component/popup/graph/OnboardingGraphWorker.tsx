...
const OnboardingGraphWorker = observer(({ 
	...
	const handleMouseLeave = (e: React.MouseEvent<HTMLCanvasElement>) => {
		...
		if (!workerRef.current || !isDragging) return;
	...
	// Initialize worker
	useEffect(() => {
		if (!canvasRef.current || workerRef.current) return; // Don't initialize if already initialized
		...
		const isDarkTheme = true; // For now, assume dark theme (can get from S.common.theme if needed)
