	...
	class PopupSettings extends React.Component<I.Popup, State> {
		...
		render() {
			...
			const cnr = [
				'side',
				'right',
				UtilCommon.toCamelCase('tab-' + page),
			];
		...
		rebind() {
			...
			this.unbind();
			win.on('resize.settings', () => {
				this.resize();
			});
			win.on('keydown.settings', e => this.onKeyDown(e));
			win.on('mousedown.settings', (e: any) => {
				this.onMouseDown(e);
			});
		}
		...
		getSections(): any[] {
			...
			if (isSpace) {
				...
				return [
					...
					{
						name: translate('popupSettingsVoidTitle'),
						children: [
							{
								id: 'dataManagement',
								name: translate(
									'popupSettingsDataManagementTitle'
								),
								icon: 'storage',
								subPages: ['delete'],
							},
		...
		onExport(type: I.ExportType, param: any) {
			...
			Action.export([], type, { ...param, route: 'Settings' }, () =>
				this.props.close()
			);
		}
		...
		onKeyDown(e: any) {
			keyboard.shortcut(
				UtilCommon.isPlatformMac() ? 'cmd+[' : 'alt+arrowleft',
				e,
				() => this.onBack()
			);
		}
