	...
	class PopupSearch extends React.Component<I.Popup, State> {
		...
		getItems() {
			...
			if (filter) {
				...
				const itemsImport: any[] = (
					[
						...
						},
						{
							...
							name: translate('popupSettingsImportProtobufTitle'),
							format: I.ImportType.Protobuf,
						},
						{
							id: 'importMarkdown',
							icon: 'import-markdown',
							name: translate('popupSettingsImportMarkdownTitle'),
							format: I.ImportType.Markdown,
						},
					] as any[]
				).map(it => ({ ...it, isImport: true }));
				...
				).map(it => ({ ...it, isSpace: true, className: 'isSpace' }));
			...
			if (hasRelations) {
				items.push({
					id: 'relation',
					name: translate('popupSearchAddRelation'),
					icon: 'relation',
		...
		onClick(e: any, item: any) {
			...
			if (item.isObject) {
				...
				Action.import(
					item.format,
					Constant.extension.import[item.format]
				);
			} else {
				switch (item.id) {
					case 'add': {
					...
					case 'relation': {
						...
						window.setTimeout(() => {
							$('#menuBlockRelationView #item-add').trigger(
								'click'
							);
						}, Constant.delay.menu * 2);
						break;
					}
				}
			}
