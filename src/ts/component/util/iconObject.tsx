	...
	class IconObject extends React.Component<Props> {
		...
		render() {
			...
			const cn = [
				'iconObject',
				'c' + size,
				UtilData.layoutClass(object.id, layout),
			];
			...
			switch (layout) {
				...
				case I.ObjectLayout.Image: {
					if (id) {
						...
						icon = (
							<img
								src={commonStore.imageUrl(id, iconSize * 2)}
								className={icn.join(' ')}
							/>
						);
					} else {
		...
		onMouseEnter(e: any) {
			...
			if (
				canEdit &&
				object.layout == I.ObjectLayout.Task &&
				!object.done
			) {
		...
		onEmoji(e: any) {
			...
			const noGallery =
				this.props.noGallery || layout == I.ObjectLayout.SpaceView;
			const noUpload =
				this.props.noUpload || layout == I.ObjectLayout.Type;
		...
		userSvg(): string {
			...
			const text = `<text x="50%" y="50%" text-anchor="middle" dominant-baseline="central" fill="${this.svgColor()}" font-family="Helvetica" font-weight="medium" font-size="${this.fontSize(
				layout,
				iconSize
			)}px">${name}</text>`;
			const svg = `
		...
		gradientSvg(radius: number): string {
			const object = this.getObject();
			const iconSize = this.iconSize();
			const option = Colors.gradientIcons.options[
				object.iconOption - 1
			] as any;
		...
		`;
			...
			const circle = `<circle cx="50%" cy="50%" r="${
				radius * 100
			}%" fill="url(#gradient)" />`;
			const svg = `
			<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 ${iconSize} ${iconSize}" xml:space="preserve" height="${iconSize}px" width="${iconSize}px">
