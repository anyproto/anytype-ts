...
const WidgetTree = observer(
	class WidgetTree extends React.Component<I.WidgetComponent, State> {
		...
		loadTree(): I.WidgetTreeItem[] {
			...
			if (isCollection(targetBlockId)) {
				...
				children = records.map(id =>
					this.mapper(
						detailStore.get(subId, id, Constant.sidebarRelationKeys)
					)
				);
			} else {
		...
		loadTreeRecursive(
			...
			const branch =
				this.branches.find(
					branchId => branchId.match(regN) || branchId.match(regS)
				) || parentId;
		...
		mapper(item) {
			item.links =
				item.layout != I.ObjectLayout.Set
					? this.filterDeletedLinks(
		...
		onToggle(e: React.MouseEvent, node: I.WidgetTreeItem): void {
			...
			analytics.event(
				!isOpen ? 'OpenSidebarObjectToggle' : 'CloseSidebarObjectToggle'
			);
			this.forceUpdate();
		}
