	...
	class WidgetListItem extends React.Component<Props> {
		...
		render() {
			...
			const object = detailStore.get(
				subId,
				id,
				Constant.sidebarRelationKeys
			);
			const { isReadonly, isArchived, restrictions, source, done } =
				object;
			const iconKey = `widget-icon-${block.id}-${id}`;
			const canDrop =
				!isEditing &&
				blockStore.isAllowed(restrictions, [I.RestrictionObject.Block]);
			const canDrag =
				isPreview &&
				block.content.targetBlockId == Constant.widgetId.favorite;
