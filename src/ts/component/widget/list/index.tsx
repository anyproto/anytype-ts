...
const WidgetList = observer(
	class WidgetList extends React.Component<Props, State> {
		node = null;
		refSelect = null;
		state = {
			isLoading: false,
		...
		render(): React.ReactNode {
			...
			const views = dbStore.getViews(rootId, BLOCK_ID).map(it => ({
				...it,
				name: it.name || UtilObject.defaultName('Page'),
			}));
			...
			if (isLoading) {
				...
				content = (
					<Label
						className="empty"
						text={translate('widgetEmptyLabel')}
					/>
				);
			} else if (isPreview) {
					...
					viewSelect = (
						<div className="viewList">
							<div className="inner">
								{views.map(item => (
									<Item
										key={`widget-${block.id}-view-${item.id}`}
										{...item}
									/>
								))}
							</div>
			...
			return (
				<div ref={node => (this.node = node)} className="innerWrap">
		...
		componentDidMount(): void {
			...
			if (isCollection(targetBlockId)) {
				getData(dbStore.getSubId(this.getRootId(), BLOCK_ID), () =>
					this.resize()
				);
			} else {
		...
		updateViews() {
			...
			const { targetBlockId } = block.content;
			const views = UtilCommon.objectCopy(
				dbStore.getViews(targetBlockId, BLOCK_ID)
		...
		onSortEnd(result: any) {
			...
			if (
				oldIndex == newIndex ||
				targetBlockId != Constant.widgetId.favorite
			) {
				return;
			}

			const { root } = blockStore;
			const children = blockStore.getChildren(root, root, it =>
				it.isLink()
			);
		...
		getRecords() {
			...
			const ret = Dataview.applyObjectOrder(
				...
				viewId,
				'',
				UtilCommon.objectCopy(records)
			);

			return targetBlockId == Constant.widgetId.favorite
				? sortFavorite(ret)
				: ret;
		}
