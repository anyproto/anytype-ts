...
const WidgetIndex = observer(
	class WidgetIndex extends React.Component<Props> {
		...
		render() {
			...
			const cn = [
				'widget',
				UtilCommon.toCamelCase(`widget-${I.WidgetLayout[layout]}`),
			];
			const object = this.getObject();
			const withSelect =
				!this.isCollection(targetBlockId) &&
				(!isPreview || !UtilCommon.isPlatformMac());
					...
					<div className="buttons">
						...
						{withPlus ? (
							<div className="iconWrap create">
								<Icon
									className="plus"
									tooltip={translate('widgetCreate')}
			...
			if (layout != I.WidgetLayout.Space) {
				const onClick = this.isCollection(targetBlockId)
					? this.onSetPreview
					: this.onClick;
				...
				case I.WidgetLayout.Compact: {
					content = (
						<WidgetList
							...
							{...this.props}
							{...props}
							isCompact={layout == I.WidgetLayout.Compact}
						/>
					);
					break;
				}
		...
		getTargetBlock(): I.Block {
			...
			return childrenIds.length
				? blockStore.getLeaf(widgets, childrenIds[0])
				: null;
		}
			...
			switch (targetBlockId) {
				...
				case Constant.widgetId.collection: {
					object = {
						id: targetBlockId,
						name: translate(
							UtilCommon.toCamelCase(`widget-${targetBlockId}`)
						),
					};
					break;
				}
		...
		onCreate(e: React.MouseEvent): void {
			...
			if (isSetOrCollection) {
				...
				isCollection = Dataview.isCollection(
					rootId,
					Constant.blockId.dataview
				);
			} else {
		...
		getData(subId: string, callBack?: () => void) {
			...
			UtilData.searchSubscribe(
				...
				() => {
					if (targetBlockId == Constant.widgetId.favorite) {
						let records = this.sortFavorite(
							dbStore.getRecords(subId, '')
						);

						if (!isPreview) {
							records = records.slice(
								0,
								this.getLimit(block.content)
							);
						}
		...
		sortFavorite(records: string[]): string[] {
			...
			const ids = blockStore
				.getChildren(root, root, it => it.isLink())
				.map(it => it.content.targetBlockId);

			return UtilCommon.objectCopy(records || []).sort(
				(c1: string, c2: string) => {
		...
		onSetPreview() {
			...
			if (!isPreview) {
				blockId = block.id;
				event = 'SelectHomeTab';
				data.tab = this.isCollection(targetBlockId)
					? object.name
		...
		isPlusAllowed(): boolean {
			...
			const { targetBlockId } = child?.content || {};
			const isRecent = [
				Constant.widgetId.recentOpen,
				Constant.widgetId.recentEdit,
			...
			if (UtilObject.isSetLayout(object.layout) && this.ref) {
				...
				if (
					type &&
					UtilObject.getFileLayouts().includes(type.recommendedLayout)
				) {
					allowed = false;
				}
			} else if (
				!blockStore.isAllowed(object.restrictions, [
		...
		getLimit({ limit, layout }): number {
			...
			const options = UtilMenu.getWidgetLimits(layout).map(it =>
				Number(it.id)
			);
