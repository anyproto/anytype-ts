...
const EditorPage = observer(
	class EditorPage extends React.Component<Props, State> {
		...
		render() {
			...
			const children = blockStore.getChildren(
				rootId,
				rootId,
		...
		open() {
			...
			C.ObjectOpen(
				...
				(message: any) => {
					...
					this.scrollTop = Storage.getScroll(
						'editor' + (isPopup ? 'Popup' : ''),
						rootId
					);
					this.focusTitle();
					this.setLoading(false);

					UtilCommon.getScrollContainer(isPopup).scrollTop(
						this.scrollTop
					);
		...
		focusTitle() {
			const { rootId } = this.props;
			const block = blockStore.getFirstBlock(rootId, 1, it =>
				it.isText()
			);
		...
		onMouseMove(e: any) {
			...
			const menuOpen =
				menuStore.isOpen() && !menuStore.isOpen('onboarding');
			...
			this.frameMove = raf(() => {
				...
				const buttonX =
					hoveredRect.x -
					(rectContainer.x - Constant.size.blockMenu) +
					2;
		...
		onKeyDownBlock(
			...
			keyboard.shortcut(`${cmd}+/, ctrl+shift+/`, e, () => {
				menuStore.close('blockContext', () => {
					menuStore.open('blockAction', {
						...
						data: {
							blockId: block.id,
							blockIds: UtilData.selectionGet(
								block.id,
								true,
								true,
								this.props
			...
			if (range.from == range.to) {
				keyboard.shortcut(`${cmd}+k`, e, () =>
					keyboard.onSearchPopup('Shortcut')
				);
			}
		...
		onTabEditor(e: any, ids: string[], pressed: string) {
			...
			const next = nextId
				? blockStore.getLeaf(rootId, nextId)
				: blockStore.getNextBlock(rootId, first.id, -1);
		...
		onCtrlShiftArrowEditor(e: any, pressed: string) {
			...
			const block = blockStore.getLeaf(
				rootId,
			...
			const next = blockStore.getNextBlock(
				rootId,
				block.id,
				dir,
				(it: any) => {
		...
		onCtrlArrowBlock(e: any, pressed: string) {
			...
			const { rootId } = this.props;
			const dir = pressed.match(Key.up) ? -1 : 1;
			const next = blockStore.getFirstBlock(rootId, -dir, it =>
				it.isFocusable()
			);
		...
		onTabBlock(e: any, range: I.TextRange, isShift: boolean) {
			...
			const next = nextId
				? blockStore.getLeaf(rootId, nextId)
				: blockStore.getNextBlock(rootId, block.id, -1);
		...
		menuCheck() {
			...
			return (
				menus.length > 1 ||
				(menus.length == 1 && !exclude.includes(menus[0].id))
			);
		}
		...
		getNextTableRow(id: string, dir: number) {
			...
			return element
				? blockStore.getNextBlock(rootId, element.parentId, dir, it =>
						it.isTableRow()
					)
				: null;
		}
		...
		onArrowVertical(
			...
			if (isInsideTable) {
				const element = blockStore.getMapElement(rootId, block.id);
				const rowElement = blockStore.getMapElement(
					rootId,
					element.parentId
				);
				...
				if (idx >= 0 && nextRow) {
					const nextRowElement = blockStore.getMapElement(
						rootId,
						nextRow.id
					);
					C.BlockTableRowListFill(rootId, [nextRow.id], () => {
						if (nextRowElement) {
							next = blockStore.getLeaf(
								rootId,
					...
					if (!nextCellId) {
						...
						if (nextRow) {
							...
							fill(nextRow.id, () => {
								...
								this.focusNextBlock(
									blockStore.getLeaf(rootId, nextCellId),
									dir
								);
							});
						}
		...
		onSelectAll() {
			const { dataset, rootId } = this.props;
			const { selection } = dataset || {};
			const ids = blockStore
				.getBlocks(rootId, it => it.isSelectable())
				.map(it => it.id);
		...
		onPaste(e: any, props: any, force?: boolean, data?: any) {
			...
			const { selection } = dataset || {};
			const { focused, range } = focus.state;
			const files = UtilCommon.getDataTransferFiles(
		...
		onPasteUrl(url: string, isLocal: boolean, props: any) {
			...
			const first = blockStore.getFirstBlock(
				rootId,
				1,
				it =>
			...
			menuStore.open('selectPasteUrl', {
				...
				data: {
					...
					onSelect: (event: any, item: any) => {
						const marks = UtilCommon.objectCopy(
							block.content.marks || []
						);
		...
		blockMerge(focused: I.Block, dir: number, length: number) {
			const { rootId } = this.props;
			const next = blockStore.getNextBlock(rootId, focused.id, dir, it =>
				it.isFocusable()
			);
		...
		blockRemove(focused?: I.Block) {
			...
			focus.clear(true);
			let next = blockStore.getNextBlock(rootId, blockIds[0], -1, it =>
				it.isFocusable()
			);
		...
		onLastClick(e: any) {
			...
			let last = blockStore.getFirstBlock(rootId, -1, it =>
				it.canCreateBlock()
			);
			let create = false;
			let length = 0;
		...
		resizePage() {
			...
			this.frameResize = raf(() => {
				...
				const pageContainer = UtilCommon.getPageContainer(
					this.props.isPopup
				);
				...
				if (blocks.length && last.length && scrollContainer.length) {
					...
					const height = Math.max(
						ch / 2,
						ch - blocks.outerHeight() - blocks.offset().top - ct - 2
					);

					last.css({
						height: Math.max(Constant.size.lastBlock, height),
					});
				}
		...
		isReadonly() {
			...
			const root = blockStore.getLeaf(rootId, rootId);
			const allowed = blockStore.checkFlags(rootId, rootId, [
