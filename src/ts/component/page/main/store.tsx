...
const PageMainStore = observer(
	class PageMainStore extends React.Component<I.PageComponent, State> {
		...
		render() {
			...
			const Item = (item: any) => {
				const allowedDelete = blockStore.isAllowed(item.restrictions, [
		...
		getItems() {
			...
			const records = dbStore
				.getRecords(Constant.subId.store, '')
				.map(id => detailStore.get(Constant.subId.store, id));
			const limit = this.getLimit();
		...
		resize() {
			...
			if (menuStore.isOpen(this.getMenuId())) {
				...
				menuStore.update(this.getMenuId(), {
					width: filter.outerWidth(),
				});
			}
		}
