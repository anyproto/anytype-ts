...
const PageMainHistory = observer(
	class PageMainHistory extends React.Component<I.PageComponent, State> {
				...
				return (
					<React.Fragment>
						...
						>
							...
							<div className="date">
								{UtilDate.date('d F, H:i', item.time)}
							</div>
							{item.authorName ? (
		...
		show(id: string) {
			...
			const month = groups.find(
				it => it.groupId == this.monthId(version.time)
			);
			if (!month) {
				return;
			}
		...
		groupData(versions: I.HistoryVersion[]) {
			...
			for (let i = 0; i < versions.length; ++i) {
				...
				if (
					prev &&
					(prev.time - version.time > GROUP_OFFSET ||
						prev.time - version.time < 0)
				) {
					groupId++;
				}
