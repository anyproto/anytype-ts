...
const PageMainType = observer(
	class PageMainType extends React.Component<I.PageComponent, State> {
		...
		render() {
			...
			const layout: any =
				UtilMenu.getLayouts().find(
					it => it.id == object.recommendedLayout
				) || {};
			const showTemplates =
				!UtilObject.getLayoutsWithoutTemplates().includes(
			...
			const allowedObject =
				object.isInstalled &&
				UtilObject.getPageLayouts().includes(object.recommendedLayout);
			...
			const allowedTemplate =
				object.isInstalled && allowedObject && showTemplates;
			const allowedLayout =
			...
			const relations = recommendedRelations
				.map(id => dbStore.getRelationById(id))
				.filter(it => {
					if (!it) {
						return false;
					}
			...
			const isFileType = UtilObject.isFileLayout(
				object.recommendedLayout
			);
			const columns: any[] = [
				{
			...
			if (!isFileType) {
				columns.push({
					relationKey: 'creator',
					name: translate('pageMainTypeOwner'),
					isObject: true,
				});
			}
		...
		onCreate() {
			...
			const allowedObject =
				UtilObject.getPageLayouts().includes(type.recommendedLayout) ||
				isSetLayout;
			const options = [];
