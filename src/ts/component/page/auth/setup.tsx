		...
		render() {
			...
			if (error.code) {
				if (error.code == Errors.Code.FAILED_TO_FIND_ACCOUNT_INFO) {
					content = (
						<div className="importBackupWrap">
							...
							<Title
								className="animation"
								text={translate('pageAuthSetupCongratulations')}
							/>
							<Label
								className="animation"
								text={translate('pageAuthSetupLongread')}
							/>
				...
				content = (
					<React.Fragment>
						<Title
							className="animation"
							text={translate('pageAuthSetupEntering')}
						/>
			...
			return (
				<div ref={node => (this.node = node)} className="wrapper">
					...
					<Frame ref={ref => (this.refFrame = ref)}>{content}</Frame>
				</div>
			);
		}
			...
			C.WalletRecover(walletPath, phrase, (message: any) => {
				...
				UtilData.createSession((message: any) => {
					...
					if (accountId) {
						...
						UtilRouter.go('/auth/account-select', {
							replace: true,
						});
					}
				});
