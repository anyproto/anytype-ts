...
const DragProvider = observer(
	class DragProvider extends React.Component<Props> {
		...
		onDropCommon(e: any) {
			...
			const last = blockStore.getFirstBlock(rootId, -1, it =>
				it.canCreateBlock()
			);
		...
		onDragOver(e: any) {
			...
			const diff = isPopup
				? Math.abs(top - this.top) * (top > this.top ? 1 : -1)
				: 0;
		...
		onDrop(
			...
			switch (targetType) {
				case I.DropType.Block: {
					const target = blockStore.getLeaf(
						targetContextId,
						targetId
					);
					...
					if (target) {
						...
						if (
							...
							const element = blockStore.getMapElement(
								targetContextId,
								targetId
							);
							const parent = blockStore.getLeaf(
								targetContextId,
								element.parentId
							);
		...
		checkNodes(e: any, ex: number, ey: number) {
			...
			const isItemDrag = UtilCommon.getDataTransferItems(
				dataTransfer.items
			).length
				? true
				: false;
