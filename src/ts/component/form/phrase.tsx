...
class Phrase extends React.Component<Props, State> {
	...
	updateValue() {
		...
		this.clear();
		this.setState(({ phrase }) => ({
			phrase: this.checkValue(phrase.concat([value])),
		}));
	}
	...
	checkValue(v: string[]) {
		return v
			.map(it => it.substring(0, Constant.limit.phrase.letter))
			.filter(it => it)
			.slice(0, Constant.limit.phrase.word);
	}
