...
const BlockText = observer(
	class BlockText extends React.Component<Props> {
				...
				if (_empty_) {
					...
					icon = (
						<IconObject
							...
							canEdit={!isArchived}
							onSelect={icon =>
								this.onMentionSelect(object.id, icon)
							}
							onUpload={hash =>
								this.onMentionUpload(object.id, hash)
							}
							onCheckbox={() =>
								this.onMentionCheckbox(object.id, !done)
							}
						/>
					);
				}
		...
		onKeyUp(e: any) {
			...
			const canOpenMentionMenu =
				oneSymbolBefore == '@' &&
				!this.preventMenu &&
				(isAllowedMention || range.from == 1) &&
				!keyboard.isSpecial(e) &&
				!menuOpenMention &&
				!block.isTextCode() &&
				!block.isTextTitle() &&
		...
		onSelect() {
			const { rootId, dataset, block, isPopup, isInsideTable, readonly } =
				this.props;
			...
			this.timeoutContext = window.setTimeout(() => {
				...
				this.setText(this.marks, true, () => {
					...
					window.setTimeout(() => {
						const pageContainer =
							UtilCommon.getPageContainer(isPopup);
