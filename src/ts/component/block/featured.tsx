...
const BlockFeatured = observer(
	class BlockFeatured extends React.Component<Props> {
		...
		render() {
			...
			const allowedValue = blockStore.checkFlags(rootId, rootId, [
			...
			if (tl) {
				setOfString.push(
					UtilCommon.sprintf(
						'%s: %s',
						UtilCommon.plural(tl, translate('pluralType')),
			...
			if (rl) {
				setOfString.push(
					`${UtilCommon.plural(
						rl,
						translate('pluralUCRelation')
					)}: ${relations.slice(0, SOURCE_LIMIT).join(', ')}`
				);
				...
				>
					...
					{featuredRelations.includes('setOf') ? (
						...
						>
							...
							<div
								...
								onMouseEnter={(e: any) => {
									this.onMouseEnter(e, 'setOf', 'Query');
								}}
								onMouseLeave={this.onMouseLeave}
							>
								{setOfString.length ? (
					...
					{items.map((relationKey: any, i: any) => {
						...
						if (['links', 'backlinks'].includes(relationKey)) {
							const options = object[relationKey]
								.map(it => detailStore.get(rootId, it, []))
		...
		getItems() {
			...
			return (object.featuredRelations || []).filter(
				it => dbStore.getRelationByKey(it) && !skipIds.includes(it)
			);
		}
		...
		onType(e: any) {
			...
			const showMenu = () => {
				menuStore.open('select', {
					element: `#block-${block.id} #${Relation.cellId(
						PREFIX,
						'type',
						rootId
					)}`,
		...
		onLinks(e: React.MouseEvent, relationKey: string) {
			...
			const elementId = Relation.cellId(
				PREFIX + block.id,
				relationKey,
				object.id
			);
