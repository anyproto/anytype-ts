	...
	class BlockTable extends React.Component<I.BlockComponent> {
		...
		onResizeStart(e: any, id: string) {
			...
			win.on(
				'mousemove.table',
				throttle((e: any) => {
					this.onResizeMove(e, id);
				}, 40)
			);
			win.on('mouseup.table', (e: any) => {
				this.onResizeEnd(e, id);
			});
		...
		getColumnWidths(): number[] {
			...
			columns.forEach((it: I.Block) => {
				ret.push(
					this.checkWidth(
						it.fields.width || Constant.size.table.default
					)
				);
			});
		...
		onDragStartColumn(e: any, id: string) {
			...
			win.on(
				'drag.tableColumn',
				throttle((e: any) => {
					this.onDragMoveColumn(e, id);
				}, 40)
			);
			win.on('dragend.tableColumn', (e: any) => {
				this.onDragEndColumn(e, id);
			});
		...
		onDragStartRow(e: any, id: string) {
			...
			win.on(
				'drag.tableRow',
				throttle((e: any) => {
					this.onDragMoveRow(e, id);
				}, 40)
			);
			win.on('dragend.tableRow', (e: any) => {
				this.onDragEndRow(e, id);
			});
		...
		initSize() {
			...
			const widths = columns.map(it =>
				this.checkWidth(it.fields.width || Constant.size.table.default)
			);
		...
		optionsAlign(cellId: string) {
			...
			return [
				{
					id: 'horizontal',
					icon: this.alignHIcon(current?.hAlign),
					name: translate('blockTableOptionsAlignText'),
					arrow: true,
				},
				{
					id: 'vertical',
					icon: this.alignVIcon(current?.vAlign),
			...
			return [
				{
					...
					name: translate('blockTableOptionsAlignTextLeft'),
				},
				{
					id: I.BlockHAlign.Center,
					name: translate('blockTableOptionsAlignTextCenter'),
				},
				{
					id: I.BlockHAlign.Right,
					name: translate('blockTableOptionsAlignTextRight'),
				},
			].map((it: any) => {
			...
			return [
				{
					...
					name: translate('blockTableOptionsAlignVerticalTop'),
				},
				{
					id: I.BlockVAlign.Middle,
					name: translate('blockTableOptionsAlignVerticalMiddle'),
				},
				{
					id: I.BlockVAlign.Bottom,
					name: translate('blockTableOptionsAlignVerticalBottom'),
				},
			].map((it: any) => {
