...
const BlockDataview = observer(
	class BlockDataview extends React.Component<Props, State> {
		...
		render() {
			...
			const className = [
				UtilCommon.toCamelCase('view-' + I.ViewType[view.type]),
			];
				...
				body = (
					<div className="content">
						<ViewComponent
							...
							{...this.props}
							{...dataviewProps}
							bodyContainer={UtilCommon.getBodyContainer(
								isPopup ? 'popup' : 'page'
							)}
							pageContainer={UtilCommon.getCellContainer(
								isPopup ? 'popup' : 'page'
							)}
		...
		componentDidUpdate() {
			...
			const { viewId } = dbStore.getMeta(
				dbStore.getSubId(rootId, block.id),
				''
			);
		...
		rebind() {
			...
			win.on(`resize.${block.id} sidebarResize.${block.id}`, () =>
				this.resize()
			);
			win.on(`updateDataviewData.${block.id}`, () =>
		...
		reloadData() {
			...
			if (view) {
				dbStore.metaSet(rootId, block.id, {
					viewId: view.id,
					offset: 0,
					total: 0,
				});
		...
		getKeys(id: string): string[] {
			...
			let keys = Constant.defaultRelationKeys.concat(
				Constant.coverRelationKeys
			);
			if (view) {
				keys = keys.concat(
				...
				if (
					...
					view.coverRelationKey != Constant.pageCoverRelationKey
				) {
		...
		getSubId(groupId?: string): string {
			...
			return groupId
				? dbStore.getGroupSubId(rootId, block.id, groupId)
				: dbStore.getSubId(rootId, block.id);
		}
		...
		getTarget() {
			...
			return detailStore.get(rootId, isInline ? targetObjectId : rootId, [
				'setOf',
			]);
		}
		...
		onTemplateMenu(e: any, dir: number) {
			...
			menuStore.open('dataviewTemplateList', {
				...
				data: {
					...
					onSetDefault: item => {
						C.BlockDataviewViewUpdate(rootId, block.id, view.id, {
							...view,
					...
					onSelect: (item: any) => {
						...
						if (
							type &&
							type.uniqueKey == Constant.typeKey.bookmark
						) {
							menuContext.close();
							this.onBookmarkMenu(e, dir, '', {
								element: `#button-${block.id}-add-record`,
							});
						} else if (item.id == Constant.templateId.new) {
							...
							C.BlockDataviewViewUpdate(
								...
								block.id,
								view.id,
								{ ...view, defaultTemplateId: item.id }
							);
		...
		onCellChange(
			...
			const key = Relation.checkRelationValue(relation, value)
				? 'ChangeRelationValue'
				: 'DeleteRelationValue';
		...
		onSourceSelect(element: any, param: Partial<I.MenuParam>) {
			...
			if (isCollection) {
				...
				addParam.onClick = () => {
					C.ObjectCreateSet([], {}, '', commonStore.space, message =>
			...
			switch (type) {
				case 'target': {
					...
					emptyProps = {
						...
						description: UtilCommon.sprintf(
				...
				case 'view': {
					...
					if (this.isAllowedObject()) {
						...
						emptyProps.button = translate(
							'blockDataviewEmptyViewButton'
						);
		...
		isAllowedObject() {
			...
			let isAllowed =
				!readonly &&
				blockStore.checkFlags(rootId, block.id, [
