	...
	class Block extends React.Component<Props> {
		...
		render() {
			...
			if (block.isLayoutColumn() && canDrop) {
				...
				const lastId = childrenIds.length
			...
			if (canSelect) {
				...
				object = (
					<div id={`selectable-${id}`} className="selectable">
						{object}
					</div>
				);
			}
		...
		onMenuDown(e: any) {
			...
			focus.clear(true);
			this.ids = UtilData.selectionGet(
				this.props.block.id,
				false,
				false,
				this.props
		...
		onResizeStart(e: any, index: number) {
			...
			win.on('mousemove.block', (e: any) => {
				this.onResize(e, index, offset);
			});
			win.on('mouseup.block', (e: any) => {
				this.onResizeEnd(e, index, offset);
			});
