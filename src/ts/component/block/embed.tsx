...
const BlockEmbed = observer(
	class BlockEmbed extends React.Component<I.BlockComponent, State> {
		...
		render() {
			...
			const menuItem: any = UtilMenu.getBlockEmbed().find(
				it => it.id == processor
			) || { name: '', icon: '' };
			...
			switch (processor) {
				default: {
					...
					placeholder = UtilCommon.sprintf(
						translate('blockEmbedPlaceholder'),
						menuItem.name
					);

					if (!text) {
						empty = UtilCommon.sprintf(
					...
					if (
						!isShowing &&
						text &&
						!UtilEmbed.allowAutoRender(processor)
					) {
						cn.push('withPreview');
					}
					break;
		...
		onScroll() {
			...
			if (
				!this._isMounted ||
				!UtilEmbed.allowScroll(processor) ||
				UtilEmbed.allowAutoRender(processor)
			) {
				return;
			}
			...
			const menuParam = {
				...
				data: {
					...
					onSelect: (from: number, to: number, item: any) => {
						...
						const value = UtilCommon.stringInsert(
							this.getValue(),
							text,
							from,
							to
			...
			switch (processor) {
				default: {
					...
					const allowIframeResize =
						UtilEmbed.allowIframeResize(processor);
					...
					const onLoad = () => {
						...
						const data: any = {
							...
							allowIframeResize,
							insertBeforeLoad:
								UtilEmbed.insertBeforeLoad(processor),
						...
						if (
							UtilEmbed.allowEmbedUrl(processor) &&
							!text.match(/<(iframe|script)/)
						) {
							text = UtilEmbed.getHtml(
								processor,
								UtilEmbed.getParsedUrl(text)
							);
						}
					...
					if (!iframe.length) {
						iframe = $('<iframe />', {
							id: 'receiver',
							src: this.fixAsarPath(
								`./embed/iframe.html?theme=${commonStore.getThemeClass()}`
							),
							frameborder: 0,
							scrolling: 'no',
				...
				case I.EmbedProcessor.Latex: {
					value.html(
						katex.renderToString(this.text, {
							displayMode: true,
							throwOnError: false,
							output: 'html',
							trust: (context: any) =>
