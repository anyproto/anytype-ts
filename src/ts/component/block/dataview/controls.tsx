...
const Controls = observer(
	class Controls extends React.Component<Props> {
		...
		render() {
			...
			const allowedView = blockStore.checkFlags(rootId, block.id, [
			...
			const isAllowedTemplate =
				UtilObject.isAllowedTemplate(getTypeId()) ||
				(target && UtilObject.isSetLayout(target.layout) && hasSources);
		...
		onButton(element: string, component: string) {
			...
			const param: any = {
				...
				onBack: id => {
					menuStore.replace(id, component, {
						...param,
						noAnimation: true,
					});
					window.setTimeout(
						() =>
							menuStore.update(component, { noAnimation: false }),
						50
					);
				},
				data: {
