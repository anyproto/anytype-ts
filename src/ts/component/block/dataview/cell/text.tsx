...
const CellText = observer(
	class CellText extends React.Component<I.Cell, State> {
		...
		render() {
			...
			if (isEditing) {
				if (isLongText) {
					EditorComponent = (item: any) => <span>{value}</span>;
				} else if (isDate) {
				...
				if (isDate) {
					if (value !== null) {
						...
						const date = day
							...
							: UtilDate.date(
									UtilDate.dateFormat(
										viewRelation.dateFormat
									),
									value
								);
						const time = UtilDate.date(
							UtilData.timeFormat(viewRelation.timeFormat),
							value
						);
		...
		componentDidUpdate() {
			...
			if (isEditing) {
				...
				if (relation.relationKey == 'name') {
					...
					value =
						this.value !== null
							? UtilDate.date(format.join(' ').trim(), this.value)
							: '';
		...
		onBlur(e: any) {
			...
			if (
				JSON.stringify(record[relation.relationKey]) ===
