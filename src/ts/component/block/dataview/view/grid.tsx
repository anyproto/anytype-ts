...
const ViewGrid = observer(
	class ViewGrid extends React.Component<I.ViewComponent> {
		...
		render() {
			...
			const { offset, total } = dbStore.getMeta(
		...
		getColumnWidths(relationKey: string, width: number): any {
			...
			relations.forEach(it => {
				const relation: any =
					dbStore.getRelationByKey(it.relationKey) || {};
				const w =
		...
		cellPosition(cellId: string) {
			...
			const container = $(
				UtilCommon.getBodyContainer(isPopup ? 'popup' : 'page')
			);
		...
		onResizeStart(e: any, relationKey: string) {
			...
			win.on('mousemove.cell', (e: any) => {
				this.onResizeMove(e, relationKey);
			});
			win.on('mouseup.cell', (e: any) => {
				this.onResizeEnd(e, relationKey);
			});
		...
		onCellAdd(e: any) {
			...
			menuStore.open('dataviewRelationList', {
				...
				data: {
					...
					blockId: block.id,
					onAdd: () => {
						menuStore.closeAll(Constant.menuIds.cellAdd);
					},
				},
			});
		}
		...
		resize() {
			...
			const length = dbStore.getRecords(
				dbStore.getSubId(rootId, block.id),
				''
			).length;
				...
				if (parent) {
					if (parent.isPage() || parent.isLayoutDiv()) {
						...
						const vw = parentObj.length
							? parentObj.width() - Constant.size.blockMenu
							: 0;
