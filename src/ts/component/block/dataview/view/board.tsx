...
const ViewBoard = observer(
	class ViewBoard extends React.Component<I.ViewComponent> {
		...
		rebind() {
			...
			node.find('#scroll').on('scroll', (e: any) => {
				this.onScrollView();
			});
		}
		...
		loadGroupList() {
			const { rootId, block, getView, getTarget, isCollection } =
				this.props;
			...
			const el = block.content.groupOrder.find(
					...
					const groups = (message.groups || []).map((it: any) => {
						...
						switch (relation.format) {
							case I.RelationType.MultiSelect:
								...
								if (value.length) {
									option = detailStore.get(
										Constant.subId.option,
										value[0]
									);
		...
		getGroups(withHidden: boolean) {
			const { rootId, block } = this.props;
			let groups = this.applyGroupOrder(
				UtilCommon.objectCopy(dbStore.getGroups(rootId, block.id))
			);
		...
		onDragStartColumn(e: any, groupId: string) {
			...
			win.on('drag.board', (e: any) => {
				this.onDragMoveColumn(e, groupId);
			});
			win.on('dragend.board', (e: any) => {
				this.onDragEndColumn(e, groupId);
			});
		}
		...
		onDragEndCard(e: any, record: any) {
			...
			const view = getView();
			const oldSubId = dbStore.getGroupSubId(
				rootId,
				block.id,
				current.groupId
			);
			const newSubId = dbStore.getGroupSubId(
				rootId,
				block.id,
				this.newGroupId
			);
			const newGroup = dbStore.getGroup(
				rootId,
				block.id,
				this.newGroupId
			);
						...
						orders = [
							{
								...
								objectIds: dbStore.getRecords(oldSubId, ''),
							},
							{
								viewId: view.id,
								groupId: this.newGroupId,
								objectIds: dbStore.getRecords(newSubId, ''),
							},
						];
