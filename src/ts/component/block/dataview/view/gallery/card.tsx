...
const Card = observer(
	class Card extends React.Component<Props> {
		...
		render() {
			...
			const cn = [
				'card',
				UtilData.layoutClass(record.id, record.layout),
				UtilData.cardSizeClass(cardSize),
			];
		...
		mediaCover(item: any) {
			const { layout, coverType, coverId, coverX, coverY, coverScale } =
				item;
						...
						mc = (
							<MediaAudio
								playlist={[
									{
										name: item.name,
										src: commonStore.fileUrl(item.id),
									},
								]}
							/>
						);
