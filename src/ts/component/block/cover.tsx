	...
	class BlockCover extends React.Component<I.BlockComponent, State> {
		...
		render() {
			...
			const object = detailStore.get(
				rootId,
				rootId,
				['iconImage', 'iconEmoji'].concat(Constant.coverRelationKeys),
				true
			);
				...
				author = (
					<Label
						...
						text={UtilCommon.sprintf(
							...
							`<a href=${
								image.mediaArtistURL + Url.unsplash.utm
							}>${image.mediaArtistName}</a>`,
							`<a href=${
								Url.unsplash.site + Url.unsplash.utm
							}>Unsplash</a>`
						)}
					/>
				);
				...
				data: {
					...
					onSelect: (icon: string) => {
						UtilObject.setIcon(rootId, icon, '', () => {
							menuStore.update('smile', {
								element: `#block-icon-${rootId}`,
							});
						});
					},
		...
		onDragStart(e: any) {
			...
			win.on('mousemove.cover', (e: any) => {
				this.onDragMove(e);
			});
			win.on('mouseup.cover', (e: any) => {
				this.onDragEnd(e);
			});
		}
