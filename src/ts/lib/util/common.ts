...
class UtilCommon {
	...
	fromCamelCase(str: string, symbol: string) {
		return str.replace(
			/([A-Z]{1})/g,
			(str: string, p1: string) => symbol + p1.toLowerCase()
		);
	}
	...
	copyToast(label: string, text: string) {
		...
		Preview.toastShow({
			text: this.sprintf(translate('toastCopy'), label),
		});
	}
	...
	checkError(code: number): boolean {
		...
		if (
			[
				Errors.Code.ANYTYPE_NEEDS_UPGRADE,
				Errors.Code.PROTOCOL_NEEDS_UPGRADE,
			].includes(code)
		) {
			this.onErrorUpdate();
			return false;
	...
	intercept(obj: any, change: any) {
		return JSON.stringify(change.newValue) ===
			JSON.stringify(obj[change.name])
			? null
			: change;
	}
		...
		for (const item of items) {
			if (item.path) {
				...
				reader.onload = () => {
					ret.push({
						name: item.name,
						path: window.Electron.fileWrite(
							item.name,
							reader.result,
							{ encoding: 'binary' }
						),
	...
	sanitize(s: string): string {
		...
		return DOMPurify.sanitize(s, {
			...
			ALLOWED_URI_REGEXP:
				/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|xxx|file):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i,
		});
	}
}
