...
class UtilObject {
	...
	getSpaceview(id?: string) {
		return detailStore.get(
			Constant.subId.space,
			id || blockStore.spaceview
		);
	}
	...
	getProfile() {
		...
		return id
			? detailStore.get(Constant.subId.profile, this.getIdentityId())
			: null;
	}
	...
	create(
		...
		if (details.type) {
			...
			if (type) {
				...
				if (!templateId) {
					templateId =
						type.defaultTemplateId || Constant.templateId.blank;
				}
			}
		}
	...
	name(object: any) {
		...
		} else if (layout == I.ObjectLayout.Note) {
	...
	getSetLayouts(): I.ObjectLayout[] {
		return [I.ObjectLayout.Set, I.ObjectLayout.Collection];
	}

	getLayoutsWithoutTemplates(): I.ObjectLayout[] {
		return [I.ObjectLayout.Bookmark]
