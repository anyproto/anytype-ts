...
class UtilData {
	blockTextClass(v: I.TextStyle): string {
		return UtilCommon.toCamelCase(
			'text-' + String(I.TextStyle[v] || 'paragraph')
		);
	}
	...
	layoutClass(id: string, layout: I.ObjectLayout) {
		...
		switch (layout) {
			default:
				c = UtilCommon.toCamelCase('is-' + I.ObjectLayout[layout]);
				break;
	...
	threadColor(s: I.ThreadStatus) {
		...
		switch (s) {
			default:
				c = '';
				break;
			case I.ThreadStatus.Syncing:
				c = 'orange';
				break;
			case I.ThreadStatus.Failed:
			case I.ThreadStatus.Incompatible:
				c = 'red';
				break;
			case I.ThreadStatus.Synced:
				c = 'green';
				break;
		}
		return c;
	...
	onAuth(param?: any, callBack?: () => void) {
		...
		const routeParam = Object.assign(
			{ replace: true },
			(param || {}).routeParam || {}
		);
				...
				filters: [
					{
						...
						value: UtilObject.getIdentityId(),
					},
				],
				noDeps: true,
	...
	blockInsertText(
		...
		const text = UtilCommon.stringInsert(
			block.content.text,
			needle,
			from,
			to
	...
	checkDetails(rootId: string, blockId?: string) {
		...
		const object = detailStore.get(
			rootId,
			blockId,
			['layoutAlign', 'templateIsBundled'].concat(
	...
	sortMapper(it: any) {
		if (
			['lastModifiedDate', 'lastOpenedDate', 'createdDate'].includes(
				it.relationKey
			)
		) {
			it.includeTime = true;
	...
	setWindowTitle(rootId: string, objectId: string) {
		this.setWindowTitleText(
			UtilObject.name(detailStore.get(rootId, objectId, []))
		);
	}
