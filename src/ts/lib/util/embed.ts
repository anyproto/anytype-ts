...
class UtilEmbed {
	getHtml(processor: I.EmbedProcessor, content: any): string {
		const fn = UtilCommon.toCamelCase(
	...
	getParsedUrl(url: string): string {
		...
		switch (processor) {
			...
			case I.EmbedProcessor.GoogleMaps: {
				...
				url = `https://www.google.com/maps/embed/v1/${endpoint}?${new URLSearchParams(
					search
				).toString()}`;
				break;
			}
			...
			case I.EmbedProcessor.Figma: {
				url = `https://www.figma.com/embed?embed_host=share&url=${encodeURIComponent(
					url
				)}`;
				break;
			}
			...
			case I.EmbedProcessor.OpenStreetMap: {
				...
				if (coords && coords.length) {
					...
					url = `https://www.openstreetmap.org/export/embed.html?bbox=${encodeURIComponent(
						[lon, lat, lon, lat].join(',')
					)}&amp;layer=mapnik`;
				}
				break;
			}
	...
	getEnvironmentContent(processor: I.EmbedProcessor): {
		html: string;
		libs: string[];
	} {
		const libs = [];
