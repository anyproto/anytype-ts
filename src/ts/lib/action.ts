...
class Action {
	...
	download(block: I.Block, route: string) {
		...
		const url = block.isFileImage()
			? commonStore.imageUrl(hash, 1000000)
			: commonStore.fileUrl(hash);
	...
	restoreFromBackup(
		...
		this.openFile(['zip'], paths => {
			C.AccountRecoverFromLegacyExport(
				paths[0],
				walletPath,
				UtilCommon.rand(1, Constant.iconCnt),
				(message: any) => {
		...
		ids.forEach((id: string) => {
			...
			if (block && block.isTable()) {
				blocks = blocks.concat(
					blockStore.unwrapTree([
						blockStore.wrapTree(rootId, block.id),
					])
				);
			}
		});
	...
	removeSpace(id: string, route: string, callBack?: (message: any) => void) {
		...
		popupStore.open('confirm', {
			data: {
				title: UtilCommon.sprintf(
					translate('spaceDeleteWarningTitle'),
					deleted.name
				),
				...
				colorConfirm: 'red',
				onConfirm: () => {
					analytics.event('ClickDeleteSpaceWarning', {
						type: 'Delete',
					});
							...
							if (!message.error.code) {
								Preview.toastShow({
									text: UtilCommon.sprintf(
										translate('spaceDeleteToast'),
										deleted.name
									),
								});
								analytics.event('DeleteSpace', {
									type: deleted.spaceType,
								});
							}
						});
					};
	...
	createWidgetFromObject(
		...
		if (object && !object._empty_) {
			...
			} else if (UtilObject.isSetLayout(object.layout)) {
				layout = I.WidgetLayout.Compact;
			} else if (UtilObject.isPageLayout(object.layout)) {
