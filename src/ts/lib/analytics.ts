...
class Analytics {
	...
	isAllowed(): boolean {
		...
		return (
			!(
				config.sudo ||
				['alpha', 'beta'].includes(config.channel) ||
	...
	event(code: string, data?: any) {
		...
		switch (code) {
			...
			case 'DeleteWidget': {
				if (data.target) {
					data.type = Constant.widgetId[data.target.id]
						? data.target.name
						: this.typeMapper(data.target.type);
					delete data.target;
				}
			...
			case 'ClickOnboardingTooltip': {
				data.id = data.id
					? UtilCommon.toUpperCamelCase(`-${data.id}`)
					: '';
				data.type = data.type
					? UtilCommon.toUpperCamelCase(`-${data.type}`)
					: '';
				break;
			}
	...
	settingsMapper(params: any): string {
		...
		return code
			? UtilCommon.toUpperCamelCase([prefix, code].join('-'))
			: '';
	}
