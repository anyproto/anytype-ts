...
class Dispatcher {
			...
			switch (type) {
				case 'accountShow': {
					authStore.accountAdd(
						Mapper.From.Account(data.getAccount())
					);
					break;
				}

				case 'accountUpdate': {
					authStore.accountSetStatus(
						Mapper.From.AccountStatus(data.getStatus())
					);
					break;
				}
				...
				case 'accountDetails': {
					detailStore.update(
						Constant.subId.profile,
						{
							id: UtilObject.getIdentityId(),
				...
				case 'accountConfigUpdate': {
					commonStore.configSet(
						Mapper.From.AccountConfig(data.getConfig()),
						true
					);
					Renderer.send(
						'setConfig',
						UtilCommon.objectCopy(commonStore.config)
					);
					break;
				}
				...
				case 'threadStatus': {
					authStore.threadSet(rootId, {
						...
						accounts: (data.getAccountsList() || []).map(
							Mapper.From.ThreadAccount
						),
					});
					break;
				}
				...
				case 'objectRelationsAmend': {
					id = data.getId();
					dbStore.relationsSet(
						rootId,
						id,
						(data.getRelationlinksList() || []).map(
				...
				case 'objectRelationsRemove': {
					id = data.getId();
					dbStore.relationListDelete(
						rootId,
						id,
						data.getRelationkeysList() || []
					);
					break;
				}

				case 'objectRestrictionsSet': {
					blockStore.restrictionsSet(
						rootId,
						Mapper.From.Restrictions(data.getRestrictions())
					);
					break;
				}
				...
				case 'fileLocalUsage': {
					commonStore.spaceStorageSet({
						localUsage: data.getLocalbytesusage(),
					});
					break;
				}
				...
				case 'fileLimitReached': {
					...
					const percentageUsed = Math.floor(
						UtilCommon.getPercent(bytesUsed, bytesLimit)
					);
					...
					if (percentageUsed >= 99) {
						...
						Preview.toastShow({
							text: "Your local storage exceeds syncing limit. Locally stored files won't be synced",
						});
					}
					break;
				}
				...
				case 'blockSetFields': {
					...
					blockStore.update(rootId, id, {
						fields: data.hasFields()
							? Decode.struct(data.getFields())
							: {},
					});
					break;
				}
				...
				case 'blockSetLink': {
					...
					if (data.hasDescription()) {
						block.content.description = data
							.getDescription()
							.getValue();
					}

					if (data.hasTargetblockid()) {
						block.content.targetblockId = data
							.getTargetblockid()
							.getValue();
					}

					if (data.hasRelations()) {
						block.content.relations =
							data.getRelations().getValueList() || [];
					}
				...
				case 'blockSetText': {
					...
					if (data.hasMarks()) {
						content.marks = (
							data.getMarks().getValue().getMarksList() || []
						).map(Mapper.From.Mark);
					}
				...
				case 'blockSetBookmark': {
					...
					if (data.hasTargetobjectid()) {
						block.content.targetObjectId = data
							.getTargetobjectid()
							.getValue();
					}
				...
				case 'blockDataviewViewUpdate': {
					...
					keys.forEach(key => {
						const items =
							data[
								UtilCommon.toCamelCase(`get-${key.id}-list`)
							]() || [];
							...
							if (item.hasUpdate()) {
								...
								if (op.hasItem()) {
									const idx = list.findIndex(
										it => it[key.idField] == op.getId()
									);
									const item = Mapper.From[key.mapper](
										op.getItem()
									);
				...
				case 'blockDataviewObjectOrderUpdate': {
					...
					changes.forEach(it => {
						...
						const idx = afterId
							? el.objectIds.indexOf(afterId) + 1
							: 0;
					...
					blockStore.updateContent(rootId, id, {
				...
				case 'notificationSend': {
					const item = new M.Notification(
						Mapper.From.Notification(data.getNotification())
					);
	...
	detailsUpdate(
		...
		this.getUniqueSubIds(subIds).forEach(subId =>
			detailStore.update(subId, { id, details }, clear)
		);
	...
	sort(c1: any, c2: any) {
		const idx1 = SORT_IDS.findIndex(
			it => it == this.eventType(c1.getValueCase())
		);
		const idx2 = SORT_IDS.findIndex(
			it => it == this.eventType(c2.getValueCase())
		);
			...
			this.service[ct](
				...
				(error: any, response: any) => {
					...
					if (message.error.code) {
						...
						message.error.description = UtilCommon.translateError(
							type,
							message.error
						);
					}
					...
					if (debug && !SKIP_IDS.includes(type)) {
						...
						console.log(
							config.debug.js ? JSON.stringify(d, null, 3) : d
						);
					}
	...
	checkLog(type: string) {
		...
		if (
			debugCommon &&
			!['threadStatus', 'fileLocalUsage', 'fileSpaceUsage'].includes(type)
		) {
			check = true;
		}
