...
class Dataview {
	viewGetRelations(
		...
		let relations = UtilCommon.objectCopy(
	...
	getData(param: any, callBack?: (message: any) => void) {
		...
		const filters = UtilCommon.objectCopy(view.filters).concat(
			param.filters || []
		);
		const sorts = UtilCommon.objectCopy(view.sorts).concat(
			param.sorts || []
		);
	...
	getView(rootId: string, blockId: string, viewId?: string): I.View {
		...
		viewId =
			viewId ||
			dbStore.getMeta(dbStore.getSubId(rootId, blockId), '').viewId;
	...
	isCollection(rootId: string, blockId: string): boolean {
		...
		const target = targetObjectId
			? detailStore.get(rootId, targetObjectId, ['layout'], true)
			: null;

		return target
			? target.layout == I.ObjectLayout.Collection
			: isCollection;
	}
	...
	getTypeId(
		...
		const isAllowedDefaultType =
			this.isCollection(rootId, blockId) ||
		...
		if (types.length) {
			...
			for (const item of relations) {
				if (item.objectTypes.length) {
					...
					if (
						first &&
						!UtilObject.isFileLayout(first.recommendedLayout) &&
						!UtilObject.isSystemLayout(first.recommendedLayout)
					) {
						typeId = first.id;
						break;
					}
