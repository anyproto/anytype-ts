...
export default () => {
	...
	return [
		...
		text(''),
		text('Some features didn’t make it into last week’s release – so here we are again. This update brings a long-awaited request, plus a few practical improvements and smaller fixes we wanted to share with you as soon as possible.'),
		text(``),

		h2(`Highlights`),

		h3(`Invite Links Without Approval`),
		text(`One of your most requested features has arrived. You can finally add ${hl('new')} members to your space without manual approval: just share an invite link with Editor or Viewer access, and you're all set.`),
		text(`How it works:`),
		bullet(`Each space can have only <b>one active invite link</b> at a time.`),
		bullet(`<b>Editor/Viewer links stay the same</b> – switching between them doesn’t generate a new link, but new members will get the role currently selected in space settings.`),
		bullet(`Switching to <b>Request access</b> generates a new link and <b>disables previous</b> Editor/Viewer links.`),
		...
		text(`The new interface displays up to 6 templates in the Edit Type sidebar and adds a visual <b>horizontal swiper</b> for browsing more.`),
		...
		bullet(`<b>My Sites has</b> been moved to a higher level under the <b>Data Management</b> section.`),
		...
		text(`New indicators are here to keep you in the loop about your remote storage usage, with banners appearing from 55% usage and showing your current percentage in the Sync Status pop-up.`),
		text(``),

		text(`<b>Copy Identity from Member Pop-up</b>`),
		text(`You can now copy a member’s identity (or their name.any) from the space member pop-up card.`),
		text(``),

		text(`<b>App Update Banner</b>`),
		text(`The <b>Update available</b> banner is now a bit more noticeable: it appears in the bottom-left corner, even when the sidebar is closed, so you don’t miss any-updates 🤍`),
		text(``),
		...
		bullet(`Navigating back to a Query with the < button now restores the previously open view. Thanks to @${link('https://community.anytype.io/t/26928', 'Shampra')}!`),
		bullet(`After adding a bookmark to the Editor, pressing Enter now adds a new block so you can continue writing without interruption.`),
		bullet(`Fixed unreadable "Out of storage" indicator in the Sync Status pop-up when using dark mode. Thanks to @${link('https://community.anytype.io/t/28543', 'Alan71')}!`),
