...
export default () => {
	...
	return [
		...
		bullet(`Gallery cards with covers no longer "jump" briefly when opening queries in gallery layout. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		...
		bullet(`The last space in Vault is no longer hidden by the gradient. Thanks to @${link('https://community.anytype.io/t/29222', 'Shampra')}!`),
		bullet(`Widget no longer changes height when switching to Kanban/Calendar layout from Compact/List.`),
		bullet(`Kanban view in Object Type widgets now loads correctly after reload. Thanks to @${link('https://community.anytype.io/t/28935', 'siousu')}!`),
		...
		bullet(`Two-column layouts (text/image): hover no longer triggers unnecessary scrollbars or column shifts. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		bullet(`Simple Table color picker shows the true active background color. Thanks to @${link('https://community.anytype.io/t/28075', 'Code-Jack')}!`),
		...
		bullet(`The control is now hidden when its sidebar is open to avoid displaying a duplicate ToC. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		bullet(`ToC control icons in a modal window render at the correct horizontal position. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		bullet(`ToC sidebar’s close button works correctly. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		bullet(`ToC control icons repositions correctly when window width changes after closing a popup.`),
		text(``),

		text(`<b>Modal Window Mode</b>`),
		bullet(`Context menu in a modal window opens next to the Editor. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		bullet(`Editor reliability restored after reopening an Object in a modal window: cut/paste/delete text continue to work as expected. Thanks to @${link('https://community.anytype.io/t/29214', 'VisualNotes')}!`),
		...
		text(`<b>PDF Export</b>`),
		bullet(`Fixed an issue where PDF export got stuck in preview mode after a file write error. Thanks to @${link('https://community.anytype.io/t/28859', 'Hexara')}!`),
		...
		bullet(`Join requests are hidden for non-owners (Editors/Viewers).`),
		text(``),
		...
		callout(`Starting with the next release, we’ll be ending support for <b>macOS Big Sur</b>. This change follows Electron’s decision to ${link('https://www.electronjs.org/blog/electron-38-0', 'drop Big Sur support beginning with version 38.0.0')}`, '⚠️'),
		text(``),
