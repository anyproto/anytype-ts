...
export default () => {
	...
	return [
		...
		text(''),
		text('This release introduces Discussions – threaded conversations you can start inside any Object, right where the work happens. Bookmarks also get their own dedicated layout with rich previews and a redesigned paste menu.'),
		text('On top of that: redesigned option menus, reaction notifications, emoji search, and a long list of fixes and quality-of-life improvements across the app.'),
		text(''),

		h2(`Discussions`),
		text(`You can now start threaded discussions inside any Object. Post a message, reply to others, and build a conversation directly where the work happens – no need to switch to a separate Chat.`),
		...
		text(`Start a discussion from the ${hl('+')} button or the ${hl('/')} slash menu inside any Object.`),
		text(``),

		h2(`Bookmark Layout`),
		text(`Bookmarks now have their own dedicated layout. Instead of inheriting the Page layout, each bookmark displays the site's preview image, a short URL with the site icon, and a prominent "Open Link" button – making it easy to see what you've saved and jump to the source.`),
		...
		text(`<b>List View: Regular Mode and Size Setting</b>`),
		text(`List view now offers a Regular mode alongside the compact default, and a new size setting lets you control how much space each row takes.`),
		text(``),
		...
		text(`You can now set a maximum file size for automatic downloads in shared Spaces. Files above the limit won't be fetched automatically, keeping your storage usage under control.`),
		text(``),

		text(`<b>Hard Delete Permission</b>`),
		text(`Space owners can now grant or restrict the ability to permanently delete objects. This gives you finer control over who can remove content from the Bin.`),
		text(``),
		...
		text(`<b>Chat Widget Unread Counter</b>`),
		text(`The Chat section in Widgets now shows an unread message counter, so you can see at a glance which conversations have new activity.`),
		text(``),
		...
		text(`<b>Prevent Duplicate Pinned Tabs</b>`),
		text(`Pinning a tab that's already open no longer creates a duplicate – the existing tab is pinned instead.`),
		text(``),
		...
		text(`<b>Chat State in Bin</b>`),
		text(`Chat Objects in the Bin now show a clear "deleted" state instead of appearing as if they're still active.`),
		text(``),

		text(`<b>Search with Non-Latin Keyboards</b>`),
		text(`The ${hl('@')} mention search now works correctly after switching to a non-Latin keyboard layout.`),
		text(``),

		text(`<b>Inline Code Cursor Positioning</b>`),
		text(`You can now place the cursor at the very first character of an inline code span without it jumping. Thanks to @${link('https://community.anytype.io/t/27826', 'spiffytech')}!`),
		text(``),

		text(`<b>Formatting No Longer Underlines Empty Space</b>`),
		text(`Applying underline or other text formatting no longer extends the style to trailing whitespace.`),
		text(``),
		...
		bullet(`The cursor can now move past folded toggles with nested content. Thanks to @${link('https://community.anytype.io/t/29086', 'Code-Jack')}!`),
		...
		bullet(`Opening another Space in a new tab no longer opens the currently active Space instead.`),
		...
		bullet(`Bookmark drag and drop no longer causes unexpected behavior.`),
		...
		bullet(`Deleted properties via Space Settings no longer remain accessible in Set views.`),
		bullet(`Clicking a template in the Type query editor no longer breaks the sidebar.`),
		...
		bullet(`New tabs can now be created in Data Spaces.`),
		...
		bullet(`Opening another Space from a pinned tab now opens the correct Space.`),
		...
		text(`<b>Notifications & Counters</b>`),
		bullet(`Muted and hidden Chats are no longer counted in the Space unread badge in the Vault.`),
		bullet(`The unread message counter now correctly appears instead of the pinned icon for pinned Spaces.`),
