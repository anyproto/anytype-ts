...
class BlockStore {
	...
	/**
	 * Gets all headers that hide a block (entire hierarchy).
	 * @param {string} rootId - The root ID.
	 * @param {string} blockId - The block ID.
	 * @returns {I.Block[]} Array of header blocks that hide this block.
	 */
	getAllHidingHeaders (rootId: string, blockId: string): I.Block[] {
		const toggles = Storage.getToggle(rootId);
		if (!toggles.length) {
			return [];
		};
		...
		for (let i = 0; i < blockIndex; i++) {
			...
			for (let j = i + 1; j < flatList.length; j++) {
				...
				if (nextBlock.id === blockId) {
					hidingHeaders.push(block);
					break;
				};
			};
		};

		return hidingHeaders;
	};
	 ...
	 * @param {string} rootId - The root ID.
	 * @param {string[]} excludeHeaders - Optional array of header IDs to treat as expanded.
	 */
	updateHeadersToggle (rootId: string, excludeHeaders?: string[]) {
		const allToggles = Storage.getToggle(rootId);
		const toggles = excludeHeaders?.length
			? allToggles.filter(id => !excludeHeaders.includes(id))
			: allToggles;
		const flatList: { block: I.Block; isNested: boolean }[] = [];
